        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {games.map(game => (
            <Link key={game.id} to={`/game/${game.id}`}>
              <div className="h-[100%] group flex items-center justify-center flex-wrap mx-auto max-w-sm rounded overflow-hidden shadow-lg px-3 border-gray-400 border transform transition duration-300 ease-in-out hover:scale-105">
                <img className="sepia group-hover:sepia-0 w-full h-48 rounded-md pt-3 object-cover" src={game.thumbnail} alt={game.title} />
